...
final class Folder {
    ...
    var sortIndex: Int = 0
    /// The content state `summary`/`summaryParts` were last generated for (fil count + newest
    /// timestamp). When this no longer matches the folder's contents, the pinned-folder hero
    /// regenerates (Pro). Empty = never generated. Keeps AI calls to one per meaningful change.
    var summarySignature: String = ""
    /// Short fragments (2–4) summarizing the folder — the pinned-folder hero renders these as the
    /// scattered stamp snippets. Generated by Pro; empty otherwise.
